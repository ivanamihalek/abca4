...
def main():
	...
	for fields in hard_landing_search(cursor, "select * from variants"):
		...
		if protein in caracterized_protein:
			[expression_molday, transport_molday] = caracterized_protein[protein]
			[expression, transport, notes] = rule_based_params(named_field, cons_data)
			print(protein, end="")
			print(f"\t{expression_molday}\t{expression+random()*0.03}", end="")
			print(f"\t{transport_molday}\t{transport+random()*0.03}\t{notes}")
